...
const wordsList = (str, subStr) => {
  ...
  const arrWordWightSubStr = [];
  strArr.forEach((word) => {
    if (word.includes(subStr)) {
      return arrWordWightSubStr.push(word);
    }
  });
  return new Set(arrWordWightSubStr);
};
 ...
 */
const getLocalDate = (date, isSeconds, isISO) => {
  const year = date.getFullYear();
  const month =
    date.getMonth() < 10 ? '0' + (date.getMonth() + 1) : date.getMonth();
  const day = date.getDate() < 10 ? '0' + date.getDate() : date.getDate();
  const hourMinSec = String(date).substring(16, 24);
  const hourMin = String(date).substring(16, 21);
  return isSeconds === true && isISO === true
    ? `${year}-${month}-${day}, ${hourMinSec}`
    : isSeconds === true && (isISO === false || isISO == undefined)
    ? `${day}.${month}.${year}, ${hourMinSec}`
    : isSeconds === false && isISO === true
    ? `${year}-${month}-${day}, ${hourMin}`
    : `${day}.${month}.${year}, ${hourMin}`;
};
let myDate = new Date();
...
const getLocalDay = (date) => {
  let newDate = new Date(date);
  let dayOnWeek = newDate.getDay();
  return dayOnWeek == 0 ? 7 : dayOnWeek;
};
...
const getDateAgo = (date, days) => {
  ...
  const milSec1 = newDate.getTime();
  const resultData = new Date(milSec1 - days * 24 * 60 * 60 * 1000);
  const day =
    resultData.getDate() < 10
      ? '0' + resultData.getDate()
      : resultData.getDate();
  const year = resultData.getFullYear();
  const month =
    resultData.getMonth() + 1 < 10
      ? '0' + (resultData.getMonth() + 1)
      : resultData.getMonth() + 1;
...
let car = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
let car2 = new Car(5000, 'FX50 AWD', 'Infinite', 2024);
